...
class ChainSuccessionConstraint(RelationTemplate):
    ...
    def _condition(self, traces: Int[torch.Tensor, "batch activities"]) -> Bool[torch.Tensor, " batch"]:
        return torch.logical_and(
            ChainResponseConstraint(activity_a=self.activity_a, activity_b=self.activity_b)(traces),
            ChainPrecedenceConstraint(activity_a=self.activity_a, activity_b=self.activity_b)(traces),
        )
