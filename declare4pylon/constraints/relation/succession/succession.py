...
class SuccessionConstraint(RelationTemplate):
    ...
    def _condition(self, traces: Int[torch.Tensor, "batch activities"]) -> Bool[torch.Tensor, " batch"]:
        return torch.logical_and(
            ResponseConstraint(activity_a=self.activity_a, activity_b=self.activity_b)(traces),
            PrecedenceConstraint(activity_a=self.activity_a, activity_b=self.activity_b)(traces),
        )
