import pytest
import torch

from declare4pylon.constraints.existence import AbsenceConstraint
from tests.constants import PAD, A

_ = PAD

traces = [[_, _, _], [A, _, _], [A, A, _], [A, A, A]]
a_counters = [0, 1, 2, 3]


@pytest.mark.parametrize(
    ("activity", "count", "traces", "expected"),
    [
        (A, 1, traces, [count < 1 for count in a_counters]),
        (A, 2, traces, [count < 2 for count in a_counters]),
        (A, 3, traces, [count < 3 for count in a_counters]),
        (A, 4, traces, [count < 4 for count in a_counters]),
    ],
    ids=[
        "Absence 1 (i.e. max 0 occurrences allowed)",
        "Absence 2 (i.e. max 1 occurrence allowed)",
        "Absence 3 (i.e. max 2 occurrences allowed)",
        "Absence 4 (i.e. max 3 occurrences allowed)",
    ],
)
def test_absence_constraint_evaluate(
    activity: int,
    count: int,
    traces: list[list[int]],
    expected: list[bool],
):
    traces_tensor = torch.tensor(traces, dtype=torch.int32)
    expected_tensor = torch.tensor(expected, dtype=torch.bool)
    constraint = AbsenceConstraint(activity=activity, count=count)
    result = constraint.evaluate(traces_tensor)
    assert torch.equal(result, expected_tensor)
