...
class ExistenceTemplate(DeclareConstraint):
    ...
    def __eq__(self, other: object) -> bool:
        """Check equality with another existence template based on the activity."""
        if not isinstance(other, self.__class__):
            return False
        return self.activity == other.activity

    def __hash__(self) -> int:
        """Return the hash based on the operator class and activity."""
        return hash((self.__class__, self.activity))
...
class ExistenceTemplateWithCount(ExistenceTemplate):
    ...
    def __eq__(self, other: object) -> bool:
        """Check equality with another existence template with count based on the activity and count."""
        if not isinstance(other, self.__class__):
            return False
        return self.activity == other.activity and self.count == other.count

    def __hash__(self) -> int:
        """Return the hash based on the operator class, activity, and count."""
        return hash((self.__class__, self.activity, self.count))
